...
const TipsContainer = ({ data }) => {    
    ...
    return (
        <div className='TipsContainer p-5 m-5 flex flex-col items-center justify-center'>
            <h2 className='text-5xl text-center font-serif'>Essential Tips for a Sustainable Lifestyle</h2>
            ...
            <div className='cardContainer p-4 sm:p-7 m-5 max-w-[75vw] items-center justify-center flex-wrap overflow-auto flex no-scrollbar max-h-[80vh] sm:max-h-[60vh]'>
                {data.map((item) => (
                    <button
                        ...
                        key={item._id} 
                        className="cards transition duration-150 ease-in-out transform hover:-translate-y-1 hover:scale-105 hover:shadow-2xl overflow-y-auto no-scrollbar w-[60%] sm:w-[25%] m-5 bg-purple-400 h-64 p-2 text-center shadow-lg border rounded-3xl"
                    >
                        {toggles[item._id] ? (
