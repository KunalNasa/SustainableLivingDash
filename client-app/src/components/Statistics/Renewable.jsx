// src/RenewableEnergyUsageChart.jsx
import React, { useEffect } from 'react';
import ApexCharts from 'apexcharts';
import { Chart as ApexChart } from 'react-apexcharts'; // Import ApexCharts from react-apexcharts

const RenewableEnergyUsageChart = () => {
  useEffect(() => {
    // Chart configuration
    const chartConfig = {
      series: [
        {
          name: "% of renewable energy usage",
          data: [25, 25.5, 26, 26.5, 27, 27.5, 28, 28.5, 29, 29.5, 30, 30.5],
        },
      ],
      chart: {
        type: "bar",
    ...
    const chart = new ApexCharts(document.querySelector("#renewable-chart"), chartConfig);
    chart.render();
  ...
  return (
    <div className="z-0 relative flex flex-col rounded-xl bg-gradient-to-r from-slate-100 to-slate-200 bg-clip-border text-gray-700 shadow-2xl m-5">
      ...
      <div className="z-10 pt-6 px-2 pb-0">
        <div id="renewable-chart"></div>
      </div>
    </div>
  );
};

export default RenewableEnergyUsageChart;
